 ...
 */
function initializeSection1(gsapInstance, ScrollTriggerInstance) {
    ...
    const setupOdometerCounterAnimation = () => {
        ...
        if (isNaN(digitHeight) || digitHeight === 0) {
            ...
            if (isNaN(digitHeight) || digitHeight === 0) {
                console.error("Section 1 Odometer: Could not determine digit height.");
                return;
            }
        }
    ...
    const setupNewArrowAnimation = () => {
        ...
        // const triggerSection = document.querySelector("#section-1"); // Already have section1Trigger

        let currentMainPath, currentVisiblePath, currentArrowHead, activeContainer = null;

        // Determine active container based on visibility and screen width
        ...
        } else if (newArrowSvgContainerMobile && getComputedStyle(newArrowSvgContainerMobile).display !== 'none') { // Fallback if desktop is hidden
            activeContainer = newArrowSvgContainerMobile;
        } else if (newArrowSvgContainer) { // Fallback to desktop if mobile is not defined
             activeContainer = newArrowSvgContainer;
        }
        ...
        gsapInstance.set(currentArrowHead, { opacity: 1, transformOrigin: "center center" });
        ...
        const adjustRightPosition = () => {
            ...
            const R_inicial = -500, W_inicial = 2560, K_slope = 0.12;
            ...
            if (newArrowSvgContainerMobile) newArrowSvgContainerMobile.style.right = `${calculatedRightValue}px`;
            setTimeout(() => ScrollTriggerInstance.refresh(), 0); // Refresh after style change
        };
        adjustRightPosition(); // Initial call
        ...
        ScrollTriggerInstance.matchMedia({
            "(max-width: 767px)": () => ScrollTriggerInstance.create({ trigger: section1Trigger, start: "top bottom+=50px", end: () => "+=" + (section1Trigger.offsetHeight * 1.1), scrub: 1.5, animation: arrowTimeline, id: "newArrowMobileStart" }),
            "(min-width: 768px)": () => ScrollTriggerInstance.create({ trigger: section1Trigger, start: "top 80%", end: () => "+=" + (section1Trigger.offsetHeight * 1.1), scrub: 1.5, animation: arrowTimeline, id: "newArrowDesktopStart" })
        });
    };
